...
const OUTPUT_SQL = "./scripts/import_music_real.sql";
...
function getTrackNumber(filename) {
  const meteoraMatch = filename.match(/ - (\d+) - /);
  if (meteoraMatch) return Number(meteoraMatch[1]);

  const normalMatch = filename.match(/^(\d+)/);
  if (normalMatch) return Number(normalMatch[1]);

  throw new Error(`Could not detect track number from: ${filename}`);
}

function cleanSongTitle(filename) {
  let title = filename.replace(/\.[^.]+$/, "");

  title = title.replace(/^Linkin Park - Meteora - \d+ - /, "");
...
for (const folder of fs.readdirSync(ALBUMS_ROOT).sort()) {
  ...
  if (!info) {
    throw new Error(`Album folder is not configured: ${folder}`);
  }

  const coverPath = path.join(folderPath, "cover.jpg");

  if (!fs.existsSync(coverPath)) {
    throw new Error(`Missing cover.jpg in: ${folderPath}`);
  }
  ...
  for (const audioFile of audioFiles) {
    const fullAudioPath = path.join(folderPath, audioFile);
    ...
    rows.push({
      ...
      durationSeconds: getDurationSeconds(fullAudioPath),
    });
  }
}
...
if (finalSql.includes("NULL")) {
  throw new Error("Generated SQL still contains NULL. Refusing to write file.");
}
...
console.log("No NULL values were generated.");